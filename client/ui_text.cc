...
void ui::Text::create(int width, int height)
{
...
void ui::Text::set_text(int line, const std::string &text)
{
    std::vector<uint32_t> pixels = {};
...
void ui::Text::set_text(int line, const std::wstring &text)
{
    std::vector<uint32_t> pixels = {};
    pixels.resize(width, 0xFFFFFFFF);
...
void ui::Text::set_background(const vector4_t &value)
{
    background = value;
}

void ui::Text::set_foreground(const vector4_t &value)
{
    foreground = value;
}

void ui::Text::set_position(const vector2i_t &value)
{
    position = value;
}

void ui::Text::set_scale(unsigned int value)
{
...
void ui::Text::draw(const ui::Text &text, const ui::Font &font)
{
    ...
    uniforms.background = text.background;
    uniforms.foreground = text.foreground;
    ...
    uniforms.glyph.x = font.get_glyph_width() * text.scale;
    uniforms.glyph.y = font.get_glyph_height() * text.scale;
    uniforms.glyph.z = font.get_texture_cwidth();
    uniforms.glyph.w = font.get_texture_cheight();
    uniforms.rect.x = text.position.x * text.scale;
    uniforms.rect.y = text.position.y * text.scale;
    uniforms.rect.z = uniforms.glyph.x * text.width;
    uniforms.rect.w = uniforms.glyph.y * text.height;
