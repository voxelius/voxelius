...
constexpr static KeyboardBits KB_DOWN       = 0x0010;
constexpr static KeyboardBits KB_UP         = 0x0020;
...
{
    if(!debug_toggles::is_sequence_await) {
...
void keyboard::update(void)
{
    ...
    if(!globals::ui_screen) {
        if(pressed_keys & KB_FORWARD)
            direction += Vector3D::dir_forward();
        if(pressed_keys & KB_BACK)
            direction += Vector3D::dir_back();
        if(pressed_keys & KB_LEFT)
            direction += Vector3D::dir_left();
        if(pressed_keys & KB_RIGHT)
            direction += Vector3D::dir_right();
        if(pressed_keys & KB_DOWN)
            direction += Vector3D::dir_down();
        if(pressed_keys & KB_UP)
            direction += Vector3D::dir_up();
    }
    ...
    if(direction[0] || direction[2]) {
        ...
        velocity.linear[0] = cxpr::lerp(velocity.linear[0], Vector3D::dot(right, direction) * PLAYER_MOVE_SPEED, PLAYER_ACCELERATE);
        velocity.linear[2] = cxpr::lerp(velocity.linear[2], Vector3D::dot(forward, direction) * PLAYER_MOVE_SPEED, PLAYER_ACCELERATE);
    }
    else {
