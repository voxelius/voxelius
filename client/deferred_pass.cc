...
void deferred_pass::init()
{
    ...
    const vfs::path_t vert_path = "/shaders/deferred_pass.vert";
    const vfs::path_t frag_path = "/shaders/deferred_pass.frag";

    if(!vfs::read_string(vert_path, source)) {
        spdlog::critical("deferred_pass: {}: load failed", vert_path.string());
        std::terminate();
    }

    if(!vert.glsl(source)) {
        spdlog::critical("deferred_pass: {}: compile failed", vert_path.string());
        std::terminate();
    }

    if(!vfs::read_string(frag_path, source)) {
        spdlog::critical("deferred_pass: {}: load failed", frag_path.string());
        std::terminate();
    }

    if(!frag.glsl(source)) {
        spdlog::critical("deferred_pass: {}: compile failed", frag_path.string());
        std::terminate();
    }
