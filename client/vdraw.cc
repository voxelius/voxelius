...
void vdraw::init(void)
{
    GLuint vert = util::compile_shader("shaders/vdraw.vert", GL_VERTEX_SHADER);
    GLuint frag = util::compile_shader("shaders/vdraw.frag", GL_FRAGMENT_SHADER);

    if(!vert || !frag) {
        spdlog::critical("vdraw: shader compile failed");
        std::terminate();
    }
    ...
    if(!program) {
        spdlog::critical("vdraw: program link failed");
        std::terminate();
    }
...
void vdraw::cube(const WorldCoord &start, const Vec3f &scale, float width, const Vec4f &color)
{
...
void vdraw::line(const WorldCoord &start, const Vec3f &scale, float width, const Vec4f &color)
{
    WorldCoord patch = start;
    patch.chunk -= view::position.chunk;
